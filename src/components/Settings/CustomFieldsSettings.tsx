...
export function CustomFieldsSettings() {
  ...
  const handleAddTab = async () => {
    if (customTabs.length >= 3) {
      setMessage({ type: 'error', text: 'Maximum 3 custom tabs allowed per pipeline' })
          ...
          <div className="border-t pt-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900">
                Custom Tabs ({customTabs.length}/3)
              </h3>
              {customTabs.length < 3 && (
                <Badge variant="secondary" className="bg-blue-50 text-blue-700">
                  {3 - customTabs.length} slots available
                </Badge>
              )}
            </div>
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-yellow-700 mt-1">
                    You have reached the maximum limit of 3 custom tabs for this pipeline. Delete an existing tab to add a new one.
                  </p>
                </div>
              </div>
            )}
