...
import { Phone, Lock, ArrowRight, Users, CheckCircle } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { supabase } from '@/lib/supabase'

interface PartnerOTPLoginProps {
  onAuthenticated: (affiliateId: string) => void
}

export function PartnerOTPLogin({ onAuthenticated }: PartnerOTPLoginProps) {
  ...
  const handleSendOTP = async () => {
    ...
    try {
      ...
      const { data: affiliate, error: affiliateError } = await supabase
        ...
        .or(`phone.eq.${mobile},phone.eq.${phoneWithCountryCode}`)
        .maybeSingle()
      ...
      if (affiliate.status !== 'Active') {
        setError('Your affiliate account is not active. Please contact administrator.')
        setIsLoading(false)
        return
      }
      ...
      const { error: dbError } = await supabase
        .from('partner_otp_verifications')
        .insert({
          mobile: mobile,
          otp: generatedOTP,
      ...
      const requestBody = {
        action: 'send_partner_otp',
        mobile: mobile,
        otp: generatedOTP
      }

      console.log('Sending Partner OTP request:', requestBody)
      ...
      console.log('Partner OTP webhook response status:', response.status)
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        onAuthenticated(otpRecords[0].affiliate_id)
      }, 1000)
    } catch (err) {
        ...
        <Card className="shadow-2xl border-0">
          <CardHeader className="space-y-4 pb-6">
            ...
            <CardTitle className="text-2xl font-bold text-center text-gray-900">
              {step === 'mobile' ? 'Partner Portal' : 'Verify OTP'}
            </CardTitle>
            <p className="text-center text-gray-600 text-sm">
              {step === 'mobile'
                ? 'Enter your registered mobile number to access your affiliate dashboard'
            ...
            {step === 'mobile' ? (
              ...
              >
                ...
                <Button
                  ...
                  className="w-full h-12 text-base font-medium bg-gradient-to-r from-blue-500 to-green-500 hover:from-blue-600 hover:to-green-600"
                >
                  {isLoading ? (
